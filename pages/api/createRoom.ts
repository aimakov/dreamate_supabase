...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        const { data: rooms_data, error: rooms_error } = await supabase.from("rooms").select();

        if (rooms_error) res.status(400).json({ message: rooms_error.message });

        const uniqueRoomCode = generateRoomCode(rooms_data.reduce((acc: string[], val) => [...acc, val.room_code], []));
