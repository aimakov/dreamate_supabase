...
const Join = (props: Props) => {
    ...
    const getRoom = async () => {
        const { data, error } = await supabase.from("rooms").select().eq("room_code", roomCode.toUpperCase());
            ...
            <Layout>
                <div className="w-full min-h-screen flex flex-col justify-center items-center font-montserrat">
                    <div className="w-[280px] h-[280px] bg-white/30 rounded-[20px] shadow-md flex flex-col justify-center items-center">
                        ...
                        <input value={roomCode} onChange={typeRoomCode} className=" p-4 rounded-[4px] mb-[32px] w-10/12 text-center uppercase text-lg" />
