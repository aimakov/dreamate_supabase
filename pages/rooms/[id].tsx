...
const Room = (props: Props) => {
  ...
  const shuffleTeams = () => {
    ...
    setPlayers(generateTeams(teamsNumber, players), (newstate) =>
      updatePlayersSupabase(newstate)
    );
  ...
  const logging = () => {
    if (shuffledPlayers.filter((player) => player.team !== undefined).length)
      console.log(
        shuffledPlayers.filter((player) => player.team !== undefined).length
      );
  };
          ...
          <div className="flex w-full flex-col gap-4 items-center">
            ...
            <div className=" w-[200px] flex gap-4 max-w-xs justify-between">
              <Checkbox
                ...
                name={"Teams"}
                onChange={() => setShowTeamsSection((prevstate) => !prevstate)}
              />
              <Checkbox
                value={showMusicSection}
                name={"Music"}
                onChange={() => setShowMusicSection((prevstate) => !prevstate)}
              />
            </div>

            <div
              ref={teamsRef}
              className={`transition-all gap-3 w-full flex flex-col items-center ${
                showTeamsSection ? " opacity-100" : " opacity-0"
              }`}
            >
                      ...
                      {players.map((player) => {
                        if (player.team === undefined)
                          return (
                            <div
                              key={player.name}
                              className="text-sm w-full flex justify-between px-4"
                            >
                              <p>{player.name}</p>{" "}
