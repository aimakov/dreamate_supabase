...
import { IoPlayOutline, IoPauseOutline, IoPlaySkipForwardOutline } from "react-icons/io5";
...
const MusicSection = ({ songs, setSongs, room_code, setModalAction, user, host }: Props) => {
    ...
    const addSong = async () => {
        ...
        if (songs?.length) {
            setSongs(
                (prevstate: any) => [...prevstate, { url: newSong, title: response.data.title, id: uniqueId() }],
                async (newState: any) => {
                    const { error } = await supabase.from("rooms").update({ songs: newState }).eq("room_code", room_code);
                    if (error) console.log(error);
                    setNewSong("");
            ...
            setSongs([{ url: newSong, title: response.data.title, id: uniqueId() }], async (newState: any) => {
                const { error } = await supabase.from("rooms").update({ songs: newState }).eq("room_code", room_code);
                if (error) console.log(error);
                setNewSong("");
            });
        ...
        <>
            <div className="flex flex-col items-center w-full gap-2">
                ...
                <div className="flex flex-col w-10/12 items-center gap-4 rounded-xl">
                    {songs?.length ? (
                        <>
                            <div className={`w-full flex flex-col py-4 rounded-xl bg-white/30 ${musicTabOpen ? "gap-4" : null}`}>
                                <ReactPlayer
                                    playing={playing}
                                    url={songs[0]?.url ?? ""}
                                    width={"320px"}
                                ...
                                {user.id === host && (
                                    <div className="flex w-full justify-center gap-20 items-center text-2xl">
                                        {playing ? (
                                            <IoPauseOutline onClick={() => setPlaying(false)} className=" hover:cursor-pointer" />
                                        ) : (
                                            <IoPlayOutline onClick={() => setPlaying(true)} className=" hover:cursor-pointer" />
                                        )}

                                        <IoPlaySkipForwardOutline onClick={showSkipSongModal} className=" hover:cursor-pointer" />
                                        {musicTabOpen ? (
                                            <SlArrowUp className="text-[22spx] hover:cursor-pointer" onClick={() => setMusicTabOpen(false)} />
                                        ) : (
                                            <SlArrowDown className="text-[22spx] hover:cursor-pointer" onClick={() => setMusicTabOpen(true)} />
                                        )}
                            ...
                            <div className="bg-white/30 rounded-xl p-3 w-full text-sm flex flex-col gap-2 max-h-[300px] no-scroll">
                                {songs?.map((song, index) => (
                                    <>
                                        <div key={song.id} className={`flex gap-2 ${index === 0 ? " font-bold border-b-[1px] pb-2 mb-1 border-black" : null}`}>
                                        ...
                                        {index === 0 && <div className="mx-auto text-[16px]">Up next:</div>}
                                    </>
                                ))}
                            </div>
                        </>
                    ) : (
                        <p className="w-full bg-white/30 p-2 text-center rounded-xl">There are no songs</p>
                    )}
                </div>
